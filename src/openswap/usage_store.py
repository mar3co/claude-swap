...
entries) and ``openswap auto`` (scheduled polling), so each learns from the
other's fetches.
...
# server's own deadline either way. Cumulative polling from openswap's own
...
# ``openswap.log`` lines "Usage fetch failed for account N: http-429,
