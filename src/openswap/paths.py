...
Mirrors claude-code's own resolution so openswap reads and writes the same files
...
Also resolves the openswap backup root, which on Linux/WSL follows the XDG Base
Directory Specification (``$XDG_DATA_HOME/openswap``) and falls back to the
...
def get_backup_root() -> Path:
    """Return the OpenSwap backup root for the current platform.

    Linux/WSL: ``$XDG_DATA_HOME/openswap`` (default ``~/.local/share/openswap``).
    macOS: ``~/Library/Application Support/OpenSwap`` (legacy
    ``~/.claude-swap-backup`` is moved here on first run).
    Windows/unknown: ``~/.claude-swap-backup``.
    ...
    if platform in (Platform.LINUX, Platform.WSL):
        ...
        if xdg:
            ...
            if xdg_path.is_absolute():
                return xdg_path / "openswap"
        return Path.home() / ".local" / "share" / "openswap"
    if platform is Platform.MACOS:
        return Path.home() / "Library" / "Application Support" / "OpenSwap"
    return get_legacy_backup_root()


# Names that any prior openswap run may have created in the backup root without
...
_THROWAWAY_PREFIXES = ("openswap.log", "claude-swap.log")
...
def migrate_legacy_backup_dir(target: Path) -> bool:
    ...
    * No flag, both paths exist → genuine collision, refuse — *unless* the
      ...
      prior openswap run may have laid down before legacy reappeared (e.g.
