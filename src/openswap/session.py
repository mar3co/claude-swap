...
# Adoption marker: this profile's mcpServers is (or was) openswap-mirrored. Gates
...
MCP_MIRROR_MARKER = ".openswap-mcp-mirror-v1"
...
MCP_DISPLACED_STASH = ".openswap-mcp-displaced.json"
...
def is_session_stale(session_dir: Path) -> bool:
    ...
    profile marked by an older openswap on this machine has a pending
...
def read_session_credentials(session_dir: Path) -> str | None:
    ...
    location stays claude's job (see the module docstring on why openswap never
...
class SessionManager:
        ...
        if pre_creds and self._has_refresh_token(pre_creds):
            ...
            if outcome.error is not None and outcome.credentials:
                ...
                raise SessionError(
                    ...
                    f"re-add it: openswap --add-account --slot {account_num}"
                )
    ...
    def _profile_matches_backup(
        ...
        """
        from openswap import oauth as _oauth
        ...
        for name in active_items:
            ...
            if dest.is_symlink():
                if name not in managed:
                    managed = [*managed, name]  # adopt: only openswap links here
                if use_symlinks:
                    try:
    ...
    def _sync_mcp_servers(self, session_dir: Path, share: bool) -> None:
        ...
        a session are overwritten the next time openswap prepares the profile.
    ...
    def _write_manifest(self, manifest_path: Path, items: list[str]) -> None:
        ...
        fd, tmp = tempfile.mkstemp(
            dir=str(manifest_path.parent), prefix=".openswap-shared-", suffix=".tmp"
        )
        try:
    ...
    def _remove_managed(dest: Path) -> None:
        """Remove a openswap-created share entry (link or copy), never user data
