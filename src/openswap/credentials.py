"""Credential storage layer for openswap.
...
# Account-scoped siblings openswap knows about, named so the unrecognized-key
...
def shared_credential_fields(credentials: str | None) -> dict | None:
    ...
    if "claudeAiOauth" in data:
        # A sibling key openswap doesn't know defaults to slot-owned (fails
        ...
        if unrecognized:
            _logger.debug(
                "Live credential has sibling keys openswap does not recognize "
...
class CredentialStore:
    ...
    # is the documented /login + `openswap add [--slot N]`, and these files are
