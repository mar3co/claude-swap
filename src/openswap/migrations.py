"""One-time, run-once data migrations for openswap.

A small, boring home for compatibility migrations so they don't pollute the
core switch/read/write flow in :mod:`openswap.switcher`. Each migration:
...
- raises :class:`~openswap.exceptions.MigrationIncomplete` (or any other
...
def migrate_windows_keyring_to_files(switcher: "ClaudeAccountSwitcher") -> bool:
    ...
    if migrated:
        print(
            f"openswap: migrated {migrated} Windows credential(s) from "
...
def migrate_macos_keyring_to_security(switcher: "ClaudeAccountSwitcher") -> bool:
    ...
    :mod:`openswap.macos_keychain`) instead of the third-party ``keyring``
    ...
    if migrated:
        print(
            f"openswap: migrated {migrated} macOS credential(s) from the keyring "
