"""Textual-based interactive TUI for openswap.

Entry point for ``openswap tui`` (and bare ``openswap`` in an interactive
terminal). Heavy imports (textual, rich) stay inside :func:`run` so the
plain CLI paths — ``openswap list``, cron's ``openswap auto --once`` — never pay
for them.
"""
...
def run(switcher: "ClaudeAccountSwitcher", start: str = "dashboard") -> int:
    ...
    ``start="watch"`` (the ``openswap watch`` command) opens directly on the
    live watch page, stacked over the dashboard.
    """
    from openswap.appearance import detect_terminal_background, drain_stdin
