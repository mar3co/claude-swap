...
lock) make it safe to run alongside an external ``openswap auto``.
...
from openswap.settings import SETTING_SPECS, load_settings, parse_model_names
from openswap.tui import data
from openswap.tui.modals import ConfirmModal
from openswap.tui.theme import Palette
    ...
    def action_toggle_live(self) -> None:
        ...
        if self._engine.dry_run:
            self.app.push_screen(
                ConfirmModal(
                    "Go live? openswap will switch your active account "
                    "automatically when the threshold is reached.\n\n"
                    "(Same behavior as running `openswap auto` in a terminal.)",
                    title="Go live",
                    yes_label="Go live",
                ),
