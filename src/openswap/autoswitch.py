...
(so cron-driven ``openswap auto --once`` ticks behave across processes), mutated
...
from openswap.json_output import SCHEMA_VERSION, USAGE_TOKEN_EXPIRED
from openswap.locking import FileLock
from openswap.poll_policy import (
...
def _window_pcts(
    ...
    correctly ignored it. Full per-model usage lives in ``openswap list``.
    """
    return {
...
class AutoSwitchEngine:
    ...
    def _freshen_target(self, number: str, email: str) -> str:
        ...
        if self.switcher.live_session_pids_for(number, email):
            # A live `openswap run` session owns this account's token in its own
    ...
    def _tick_inner(self) -> TickOutcome:
        ...
        if current is None:
            ...
            if self.switcher.has_live_login():
                # Live login exists but openswap doesn't manage it: never act —
                # a switch would overwrite it without a backup.
                self._emit(
                    NoSwitchEvent(
                        reason="unmanaged-active-account",
                        detail="run 'openswap --add-account' to include it in rotation",
                    )
                )
            else:
                self._emit(
                    NoSwitchEvent(
                        ...
                        detail="log in and run 'openswap --add-account' first",
                    )
                )
    ...
    def _perform(
        ...
        # switch path (openswap FileLock + Claude Code locks) never takes the
        # state lock.
        with self._state_lock():
            state = self._read_state()
            if trigger in ("proactive", "consume-first") and self._in_cooldown(state):
