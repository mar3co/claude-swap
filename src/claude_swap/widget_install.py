...
``cswap widget --install`` xcodebuilds the sources next to this package and
...
from claude_swap.launch_agent import (
    _launchctl,
...
from claude_swap.widget_snapshot import WIDGET_APP_NAME, widget_app_path

LABEL = "com.cswap.widget"
SCHEME = "CSwapWidget"
HOST_PRODUCT = "cswap Widget"
...
def project_dir() -> Path:
    """Directory that contains ``CSwapWidget.xcodeproj``.

    Editable checkouts keep sources at ``<repo>/macos/CSwapWidget``. A wheel
    may vendor them next to this module as ``macos_widget``.
    """
    here = Path(__file__).resolve().parent
    for base in (here, *here.parents):
        candidate = base / "macos" / "CSwapWidget"
        if (candidate / "CSwapWidget.xcodeproj").exists():
            return candidate
    vendored = here / "macos_widget"
    if (vendored / "CSwapWidget.xcodeproj").exists():
        return vendored
    raise ClaudeSwitchError(
        "Widget sources were not found. Install claude-swap from the git "
        "checkout (editable) so macos/CSwapWidget is on disk."
    )
...
def uninstall_launch_agent(home: Path | None = None, uid: int | None = None) -> dict:
    _require_macos()
    from claude_swap.launch_agent import uninstall as _uninstall
...
def install_widget(
    *,
    ...
    derived = derived or (Path.home() / "Library" / "Caches" / "cswap-widget")
    derived.mkdir(parents=True, exist_ok=True)
    log_file = derived / "xcodebuild.log"
    cmd = [
        _xcodebuild(),
        "-project",
        str(src / "CSwapWidget.xcodeproj"),
        "-scheme",
        SCHEME,
        "-configuration",
        "Release",
