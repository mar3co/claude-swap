...
OpenSwap is not on PyPI. `openswap upgrade` runs `git pull` in this checkout, then `uv tool install --editable '.[menubar]'`, then refreshes installed LaunchAgents. If you moved the clone, reinstall from the new path.
